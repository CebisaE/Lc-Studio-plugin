...
function wpcf7_file_messages( $messages ) {
	return array_merge( $messages, array(
		'upload_failed' => array(
			'description' => __( "Uploading a file fails for any reason", 'lc_studio_contact_form' ),
			'default' => __( "There was an unknown error uploading the file.", 'lc_studio_contact_form' ),
		),

		'upload_file_type_invalid' => array(
			'description' => __( "Uploaded file is not allowed for file type", 'lc_studio_contact_form' ),
			'default' => __( "You are not allowed to upload files of this type.", 'lc_studio_contact_form' ),
		),

		'upload_file_too_large' => array(
			'description' => __( "Uploaded file is too large", 'lc_studio_contact_form' ),
			'default' => __( "The uploaded file is too large.", 'lc_studio_contact_form' ),
		),

		'upload_failed_php_error' => array(
			'description' => __( "Uploading a file fails for PHP error", 'lc_studio_contact_form' ),
			'default' => __( "There was an error uploading the file.", 'lc_studio_contact_form' ),
		),
	) );
}
...
function wpcf7_file_display_warning_message( $page, $action, $object ) {
	...
	if ( ! is_dir( $uploads_dir ) or ! wp_is_writable( $uploads_dir ) ) {
		$message = sprintf(
			...
			__( 'This contact form has file uploading fields, but the temporary folder for the files (%s) does not exist or is not writable. You can create the folder or change its permission manually.', 'lc_studio_contact_form' ),
			$uploads_dir
		);
