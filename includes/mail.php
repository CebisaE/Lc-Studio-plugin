		...
		$attachments = array_filter(
			...
			function ( $attachment ) {
					...
					if ( WP_DEBUG ) {
						trigger_error(
							sprintf(
								...
								__( 'Failed to attach a file. %s is not in the allowed directory.', 'lc_studio_contact_form' ),
								$path
							),
							E_USER_NOTICE
						);
				...
				if ( ! is_readable( $path ) or ! is_file( $path ) ) {
					if ( WP_DEBUG ) {
						trigger_error(
							sprintf(
								/* translators: %s: Attachment file path. */
								__( 'Failed to attach a file. %s is not a readable file.', 'lc_studio_contact_form' ),
								$path
							),
							E_USER_NOTICE
						);
				...
				if ( 25 * MB_IN_BYTES < $total_size[$this->name] + $file_size ) {
					if ( WP_DEBUG ) {
						trigger_error(
							__( 'Failed to attach a file. The total file size exceeds the limit of 25 megabytes.', 'lc_studio_contact_form' ),
							E_USER_NOTICE
						);
					}
