	...
	public function get_default_message( $code ) {
		switch ( $code ) {
			case self::error_maybe_empty:
				return __( "There is a possible empty field.", 'lc_studio_contact_form' );
			case self::error_invalid_mailbox_syntax:
				return __( "Invalid mailbox syntax is used.", 'lc_studio_contact_form' );
			case self::error_email_not_in_site_domain:
				return __( "Sender email address does not belong to the site domain.", 'lc_studio_contact_form' );
			case self::error_html_in_message:
				return __( "HTML tags are used in a message.", 'lc_studio_contact_form' );
			case self::error_multiple_controls_in_label:
				return __( "Multiple form controls are in a single label element.", 'lc_studio_contact_form' );
			case self::error_invalid_mail_header:
				return __( "There are invalid mail header fields.", 'lc_studio_contact_form' );
			case self::error_deprecated_settings:
				return __( "Deprecated settings are used.", 'lc_studio_contact_form' );
			default:
				return '';
		}
	}
			...
			return $this->add_error( $section,
				...
				array(
					'message' =>
						...
						__( "Unavailable names (%names%) are used for form controls.", 'lc_studio_contact_form' ),
	...
	public function detect_unavailable_html_elements( $section, $content ) {
		...
		if ( preg_match( $pattern, $content ) ) {
			return $this->add_error( $section,
				...
				array(
					'message' => __( "Unavailable HTML elements are used in the form template.", 'lc_studio_contact_form' ),
		...
		foreach ( $tags as $tag ) {
			if ( false !== strpos( $tag->raw_name, '.' ) ) {
				return $this->add_error( $section,
					...
					array(
						'message' => __( "Dots are used in form-tag names.", 'lc_studio_contact_form' ),
			...
			if ( ! preg_match( '/^([0-9A-Za-z-]+):(.*)$/', $header, $matches ) ) {
				...
				and '' !== $header_value ) {
					$this->detect_invalid_mailbox_syntax(
						...
						array(
							'message' =>
								__( "Invalid mailbox syntax is used in the %name% field.", 'lc_studio_contact_form' ),
		...
		if ( '' !== $components['attachments'] ) {
			...
			if ( $max < $total_size ) {
				$this->add_error( sprintf( '%s.attachments', $template ),
					...
					array(
						'message' => __( "The total size of attachment files is too large.", 'lc_studio_contact_form' ),
	...
	public function detect_file_not_found( $section, $content ) {
		...
		or ! is_file( $path ) ) {
			return $this->add_error( $section,
				...
				array(
					'message' =>
						__( "Attachment file does not exist at %path%.", 'lc_studio_contact_form' ),
	...
	public function detect_file_not_in_content_dir( $section, $content ) {
		...
		if ( ! wpcf7_is_file_path_in_content_dir( $path ) ) {
			return $this->add_error( $section,
				...
				array(
					'message' =>
						__( "It is not allowed to use files outside the wp-content directory.", 'lc_studio_contact_form' ),
