			...
			if ( $abort ) {
				...
				if ( '' === $this->get_response() ) {
					$this->set_response( $contact_form->filter_message(
						__( "Sending mail has been aborted.", 'lc_studio_contact_form' ) )
					);
				}
			} elseif ( $this->mail() ) {
	...
	private function spam() {
		...
		if ( strlen( $user_agent ) < 2 ) {
			...
			$this->add_spam_log( array(
				'agent' => 'wpcf7',
				'reason' => __( "User-Agent string is unnaturally short.", 'lc_studio_contact_form' ),
			) );
		}
		...
		if ( ! $this->verify_nonce() ) {
			...
			$this->add_spam_log( array(
				'agent' => 'wpcf7',
				'reason' => __( "Submitted nonce is invalid.", 'lc_studio_contact_form' ),
			) );
		}
