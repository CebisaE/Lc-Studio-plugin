...
class WPCF7_REST_Controller {
	...
	const route_namespace = 'lc_studio_contact_form/v1';
					...
					'permission_callback' => function () {
						if ( current_user_can( 'wpcf7_read_contact_forms' ) ) {
							...
							return new WP_Error( 'wpcf7_forbidden',
								__( "You are not allowed to access contact forms.", 'lc_studio_contact_form' ),
								array( 'status' => 403 )
							);
						}
					},
				...
				array(
					...
					'permission_callback' => function () {
						if ( current_user_can( 'wpcf7_edit_contact_forms' ) ) {
							...
							return new WP_Error( 'wpcf7_forbidden',
								__( "You are not allowed to create a contact form.", 'lc_studio_contact_form' ),
								array( 'status' => 403 )
							);
						}
					},
				...
				array(
					...
					'permission_callback' => function ( WP_REST_Request $request ) {
						...
						if ( current_user_can( 'wpcf7_edit_contact_form', $id ) ) {
							...
							return new WP_Error( 'wpcf7_forbidden',
								__( "You are not allowed to access the requested contact form.", 'lc_studio_contact_form' ),
								array( 'status' => 403 )
							);
						}
					},
	...
	public function create_contact_form( WP_REST_Request $request ) {
		...
		if ( $id ) {
			return new WP_Error( 'wpcf7_post_exists',
				__( "Cannot create existing contact form.", 'lc_studio_contact_form' ),
				array( 'status' => 400 )
			);
		}
	...
	public function get_contact_form( WP_REST_Request $request ) {
		...
		if ( ! $item ) {
			return new WP_Error( 'wpcf7_not_found',
				__( "The requested contact form was not found.", 'lc_studio_contact_form' ),
				array( 'status' => 404 )
			);
		}
	...
	public function delete_contact_form( WP_REST_Request $request ) {
		...
		if ( ! $result ) {
			return new WP_Error( 'wpcf7_cannot_delete',
				__( "There was an error deleting the contact form.", 'lc_studio_contact_form' ),
				array( 'status' => 500 )
			);
		}
	...
	public function create_feedback( WP_REST_Request $request ) {
		...
		if ( ! str_starts_with( $content_type, 'multipart/form-data' ) ) {
			return new WP_Error( 'wpcf7_unsupported_media_type',
				__( "The request payload format is not supported.", 'lc_studio_contact_form' ),
				array( 'status' => 415 )
			);
		}
	...
	private function get_argument_schema() {
		return array(
			'id' => array(
				'description' => __( "Unique identifier for the contact form.", 'lc_studio_contact_form' ),
				'type' => 'integer',
				'required' => true,
			),
		);
