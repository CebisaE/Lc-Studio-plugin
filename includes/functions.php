		...
		if ( function_exists( '__' ) ) {
			trigger_error(
				sprintf(
					...
					__( 'Function %1$s is <strong>deprecated</strong> since Contact Form 7 version %2$s! Use %3$s instead.', 'lc_studio_contact_form' ),
		...
		if ( $replacement ) {
			trigger_error(
				sprintf(
					...
					__( 'Hook %1$s is <strong>deprecated</strong> since Contact Form 7 version %2$s! Use %3$s instead.', 'lc_studio_contact_form' ),
					$hook_name,
					$version,
					$replacement
				),
			...
			trigger_error(
				sprintf(
					...
					__( 'Hook %1$s is <strong>deprecated</strong> since Contact Form 7 version %2$s with no alternative available.', 'lc_studio_contact_form' ),
					$hook_name,
					$version
				),
				E_USER_DEPRECATED
		...
		if ( function_exists( '__' ) ) {
			if ( $version ) {
				$version = sprintf(
					...
					__( '(This message was added in Contact Form 7 version %s.)', 'lc_studio_contact_form' ),
					$version
				);
			}
			...
			trigger_error(
				sprintf(
					...
					__( 'Function %1$s was called incorrectly. %2$s %3$s', 'lc_studio_contact_form' ),
					$function_name,
					$message,
					$version
				),
...
function wpcf7_log_remote_request( $url, $request, $response ) {
	$log = sprintf(
		...
		__( 'HTTP Response: %1$s %2$s %3$s from %4$s', 'lc_studio_contact_form' ),
