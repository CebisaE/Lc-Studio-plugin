...
function wpcf7_stripe_before_send_mail( $contact_form, &$abort, $submission ) {
	...
	if ( $payment_intent ) {
		...
		$submission->set_response(
			__( "Payment is required. Please pay by credit card.", 'lc_studio_contact_form' )
		);
	}
...
function wpcf7_stripe_form_tag_handler( $tag ) {
	...
	$button_1_label = __( 'Proceed to checkout', 'lc_studio_contact_form' );
	...
	$button_2_label = __( 'Complete payment', 'lc_studio_contact_form' );
