...
function wpcf7_text_messages( $messages ) {
	$messages = array_merge( $messages, array(
		'invalid_email' => array(
			'description' =>
				__( "Email address that the sender entered is invalid", 'lc_studio_contact_form' ),
			'default' =>
				__( "Please enter an email address.", 'lc_studio_contact_form' ),
		),

		'invalid_url' => array(
			'description' =>
				__( "URL that the sender entered is invalid", 'lc_studio_contact_form' ),
			'default' =>
				__( "Please enter a URL.", 'lc_studio_contact_form' ),
		),

		'invalid_tel' => array(
			'description' =>
				__( "Telephone number that the sender entered is invalid", 'lc_studio_contact_form' ),
			'default' =>
				__( "Please enter a telephone number.", 'lc_studio_contact_form' ),
		),
	) );
...
function wpcf7_add_tag_generator_text() {
	...
	$tag_generator->add( 'text', __( 'text', 'lc_studio_contact_form' ),
		'wpcf7_tag_generator_text' );
	$tag_generator->add( 'email', __( 'email', 'lc_studio_contact_form' ),
		'wpcf7_tag_generator_text' );
	$tag_generator->add( 'url', __( 'URL', 'lc_studio_contact_form' ),
		'wpcf7_tag_generator_text' );
	$tag_generator->add( 'tel', __( 'tel', 'lc_studio_contact_form' ),
		'wpcf7_tag_generator_text' );
}
...
function wpcf7_tag_generator_text( $contact_form, $args = '' ) {
	...
	if ( 'text' == $type ) {
		$description = __( "Generate a form-tag for a single-line plain text input field. For more details, see %s.", 'lc_studio_contact_form' );
	} elseif ( 'email' == $type ) {
		$description = __( "Generate a form-tag for a single-line email address input field. For more details, see %s.", 'lc_studio_contact_form' );
	} elseif ( 'url' == $type ) {
		$description = __( "Generate a form-tag for a single-line URL input field. For more details, see %s.", 'lc_studio_contact_form' );
	} elseif ( 'tel' == $type ) {
		$description = __( "Generate a form-tag for a single-line telephone number input field. For more details, see %s.", 'lc_studio_contact_form' );
	}

	$desc_link = wpcf7_link( __( 'https://contactform7.com/text-fields/', 'lc_studio_contact_form' ), __( 'Text fields', 'lc_studio_contact_form' ) );
...
<?php if ( in_array( $type, array( 'text', 'email', 'url' ) ) ) : ?>
	<tr>
	<th scope="row"><?php echo esc_html( __( 'Akismet', 'lc_studio_contact_form' ) ); ?></th>
	<td>
		<fieldset>
		<legend class="screen-reader-text"><?php echo esc_html( __( 'Akismet', 'lc_studio_contact_form' ) ); ?></legend>

<?php if ( 'text' == $type ) : ?>
		<label>
			<input type="checkbox" name="akismet:author" class="option" />
			<?php echo esc_html( __( "This field requires author's name", 'lc_studio_contact_form' ) ); ?>
		</label>
<?php elseif ( 'email' == $type ) : ?>
		<label>
			<input type="checkbox" name="akismet:author_email" class="option" />
			<?php echo esc_html( __( "This field requires author's email address", 'lc_studio_contact_form' ) ); ?>
		</label>
<?php elseif ( 'url' == $type ) : ?>
		<label>
			<input type="checkbox" name="akismet:author_url" class="option" />
			<?php echo esc_html( __( "This field requires author's URL", 'lc_studio_contact_form' ) ); ?>
		</label>
<?php endif; ?>
