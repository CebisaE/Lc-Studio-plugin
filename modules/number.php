...
function wpcf7_number_messages( $messages ) {
	return array_merge( $messages, array(
		'invalid_number' => array(
			'description' => __( "Number format that the sender entered is invalid", 'lc_studio_contact_form' ),
			'default' => __( "Please enter a number.", 'lc_studio_contact_form' ),
		),

		'number_too_small' => array(
			'description' => __( "Number is smaller than minimum limit", 'lc_studio_contact_form' ),
			'default' => __( "This field has a too small number.", 'lc_studio_contact_form' ),
		),

		'number_too_large' => array(
			'description' => __( "Number is larger than maximum limit", 'lc_studio_contact_form' ),
			'default' => __( "This field has a too large number.", 'lc_studio_contact_form' ),
		),
	) );
}
...
function wpcf7_add_tag_generator_number() {
	...
	$tag_generator->add( 'number', __( 'number', 'lc_studio_contact_form' ),
...
function wpcf7_tag_generator_number( $contact_form, $args = '' ) {
	...
	$description = __( "Generate a form-tag for a field for numeric value input. For more details, see %s.", 'lc_studio_contact_form' );

	$desc_link = wpcf7_link( __( 'https://contactform7.com/number-fields/', 'lc_studio_contact_form' ), __( 'Number fields', 'lc_studio_contact_form' ) );
...
<tbody>
	...
	<td>
		...
		<select name="tagtype">
			<option value="number" selected="selected"><?php echo esc_html( __( 'Spinbox', 'lc_studio_contact_form' ) ); ?></option>
			<option value="range"><?php echo esc_html( __( 'Slider', 'lc_studio_contact_form' ) ); ?></option>
		</select>
		<br />
