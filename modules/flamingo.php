...
function wpcf7_flamingo_submit( $contact_form, $result ) {
	...
	if ( $channel_id ) {
		...
		if ( ! $channel or is_wp_error( $channel ) ) {
			$channel = 'lc_studio_contact_form';
		} else {
...
function wpcf7_flamingo_add_channel( $slug, $name = '' ) {
	...
	$parent = term_exists( 'lc_studio_contact_form',
		Flamingo_Inbound_Message::channel_taxonomy
	);

	if ( ! $parent ) {
		$parent = wp_insert_term( __( 'Contact Form 7', 'lc_studio_contact_form' ),
			Flamingo_Inbound_Message::channel_taxonomy,
			array( 'slug' => 'lc_studio_contact_form' )
		);
...
function wpcf7_flamingo_serial_number( $output, $name, $html, $mail_tag = null ) {
	if ( ! $mail_tag instanceof WPCF7_MailTag ) {
		wpcf7_doing_it_wrong(
			...
			__( 'The fourth parameter ($mail_tag) must be an instance of the WPCF7_MailTag class.', 'lc_studio_contact_form' ),
			'5.2.2'
		);
	}
