...
function wpcf7_add_tag_generator_checkbox_and_radio() {
	...
	$tag_generator->add( 'checkbox', __( 'checkboxes', 'lc_studio_contact_form' ),
		'wpcf7_tag_generator_checkbox' );
	$tag_generator->add( 'radio', __( 'radio buttons', 'lc_studio_contact_form' ),
		'wpcf7_tag_generator_checkbox' );
}
...
function wpcf7_tag_generator_checkbox( $contact_form, $args = '' ) {
	...
	if ( 'checkbox' == $type ) {
		$description = __( "Generate a form-tag for a group of checkboxes. For more details, see %s.", 'lc_studio_contact_form' );
	} elseif ( 'radio' == $type ) {
		$description = __( "Generate a form-tag for a group of radio buttons. For more details, see %s.", 'lc_studio_contact_form' );
	}

	$desc_link = wpcf7_link( __( 'https://contactform7.com/checkboxes-radio-buttons-and-menus/', 'lc_studio_contact_form' ), __( 'Checkboxes, radio buttons and menus', 'lc_studio_contact_form' ) );
...
<?php endif; ?>
	...
	<tr>
	<th scope="row"><?php echo esc_html( __( 'Options', 'lc_studio_contact_form' ) ); ?></th>
	<td>
		<fieldset>
		<legend class="screen-reader-text"><?php echo esc_html( __( 'Options', 'lc_studio_contact_form' ) ); ?></legend>
		<textarea name="values" class="values" id="<?php echo esc_attr( $args['content'] . '-values' ); ?>"></textarea>
		<label for="<?php echo esc_attr( $args['content'] . '-values' ); ?>"><span class="description"><?php echo esc_html( __( "One option per line.", 'lc_studio_contact_form' ) ); ?></span></label><br />
		<label><input type="checkbox" name="label_first" class="option" /> <?php echo esc_html( __( 'Put a label first, a checkbox last', 'lc_studio_contact_form' ) ); ?></label><br />
		<label><input type="checkbox" name="use_label_element" class="option" checked="checked" /> <?php echo esc_html( __( 'Wrap each item with label element', 'lc_studio_contact_form' ) ); ?></label>
<?php if ( 'checkbox' == $type ) : ?>
		<br /><label><input type="checkbox" name="exclusive" class="option" /> <?php echo esc_html( __( 'Make checkboxes exclusive', 'lc_studio_contact_form' ) ); ?></label>
<?php endif; ?>
		</fieldset>
	</td>
	</tr>
