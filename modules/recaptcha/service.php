...
class WPCF7_RECAPTCHA extends WPCF7_Service {
	...
	public function get_title() {
		return __( 'reCAPTCHA', 'lc_studio_contact_form' );
	}
	...
	public function display( $action = '' ) {
		echo '<p>' . sprintf(
			esc_html( __( 'reCAPTCHA protects you against spam and other types of automated abuse. With Contact Form 7&#8217;s reCAPTCHA integration module, you can block abusive form submissions by spam bots. For details, see %s.', 'lc_studio_contact_form' ) ),
			wpcf7_link(
				__( 'https://contactform7.com/recaptcha/', 'lc_studio_contact_form' ),
				__( 'reCAPTCHA (v3)', 'lc_studio_contact_form' )
			)
		) . '</p>';
		...
		if ( $this->is_active() ) {
			echo sprintf(
				...
				esc_html( __( "reCAPTCHA is active on this site.", 'lc_studio_contact_form' ) )
			);
		}
...
<tbody>
<tr>
	<th scope="row"><label for="sitekey"><?php echo esc_html( __( 'Site Key', 'lc_studio_contact_form' ) ); ?></label></th>
	<td><?php
		if ( $this->is_active() ) {
