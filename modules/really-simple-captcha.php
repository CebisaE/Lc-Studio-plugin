...
function wpcf7_captchac_form_tag_handler( $tag ) {
	if ( ! class_exists( 'ReallySimpleCaptcha' ) ) {
		$error = sprintf(
			...
			esc_html( __( "To use CAPTCHA, you need %s plugin installed.", 'lc_studio_contact_form' ) ),
...
function wpcf7_captcha_messages( $messages ) {
	$messages = array_merge( $messages, array(
		'captcha_not_match' => array(
			'description' =>
				__( "The code that sender entered does not match the CAPTCHA", 'lc_studio_contact_form' ),
			'default' =>
				__( 'Your entered code is incorrect.', 'lc_studio_contact_form' ),
		),
	) );
...
function wpcf7_add_tag_generator_captcha() {
	...
	$tag_generator->add( 'captcha',
		__( 'CAPTCHA (Really Simple CAPTCHA)', 'lc_studio_contact_form' ),
...
<legend><?php
	echo sprintf(
		/* translators: %s: link labeled 'Really Simple CAPTCHA' */
		esc_html( __( "To use CAPTCHA, you first need to install and activate %s plugin.", 'lc_studio_contact_form' ) ),
		wpcf7_link( 'https://wordpress.org/plugins/really-simple-captcha/', 'Really Simple CAPTCHA' )
	);
?></legend>
</fieldset>
...
<?php
	...
	$description = __( "Generate form-tags for a CAPTCHA image and corresponding response input field. For more details, see %s.", 'lc_studio_contact_form' );

	$desc_link = wpcf7_link( __( 'https://contactform7.com/captcha/', 'lc_studio_contact_form' ), __( 'CAPTCHA', 'lc_studio_contact_form' ) );
...
<caption><?php echo esc_html( __( "Image settings", 'lc_studio_contact_form' ) ); ?></caption>
<tbody>
	<tr>
	<th scope="row"><label for="<?php echo esc_attr( $args['content'] . '-captchac-id' ); ?>"><?php echo esc_html( __( 'Id attribute', 'lc_studio_contact_form' ) ); ?></label></th>
	<td><input type="text" name="id" class="idvalue oneline option" id="<?php echo esc_attr( $args['content'] . '-captchac-id' ); ?>" /></td>
	</tr>

	<tr>
	<th scope="row"><label for="<?php echo esc_attr( $args['content'] . '-captchac-class' ); ?>"><?php echo esc_html( __( 'Class attribute', 'lc_studio_contact_form' ) ); ?></label></th>
...
<caption><?php echo esc_html( __( "Input field settings", 'lc_studio_contact_form' ) ); ?></caption>
<tbody>
	<tr>
	<th scope="row"><label for="<?php echo esc_attr( $args['content'] . '-captchar-id' ); ?>"><?php echo esc_html( __( 'Id attribute', 'lc_studio_contact_form' ) ); ?></label></th>
	<td><input type="text" name="id" class="idvalue oneline option" id="<?php echo esc_attr( $args['content'] . '-captchar-id' ); ?>" /></td>
	</tr>

	<tr>
	<th scope="row"><label for="<?php echo esc_attr( $args['content'] . '-captchar-class' ); ?>"><?php echo esc_html( __( 'Class attribute', 'lc_studio_contact_form' ) ); ?></label></th>
...
function wpcf7_captcha_display_warning_message( $page, $action, $object ) {
	...
	or ! wp_is_writable( $uploads_dir ) ) {
		$message = sprintf( __( 'This contact form contains CAPTCHA fields, but the temporary folder for the files (%s) does not exist or is not writable. You can create the folder or change its permission manually.', 'lc_studio_contact_form' ), $uploads_dir );
	...
	or ! function_exists( 'imagettftext' ) ) {
		$message = __( "This contact form contains CAPTCHA fields, but the necessary libraries (GD and FreeType) are not available on your server.", 'lc_studio_contact_form' );
