...
function wpcf7_quiz_messages( $messages ) {
	$messages = array_merge( $messages, array(
		'quiz_answer_not_correct' => array(
			'description' =>
				__( "Sender does not enter the correct answer to the quiz", 'lc_studio_contact_form' ),
			'default' =>
				__( "The answer to the quiz is incorrect.", 'lc_studio_contact_form' ),
		),
	) );
...
function wpcf7_add_tag_generator_quiz() {
	...
	$tag_generator->add( 'quiz', __( 'quiz', 'lc_studio_contact_form' ),
...
function wpcf7_tag_generator_quiz( $contact_form, $args = '' ) {
	...
	$description = __( "Generate a form-tag for a question-answer pair. For more details, see %s.", 'lc_studio_contact_form' );

	$desc_link = wpcf7_link( __( 'https://contactform7.com/quiz/', 'lc_studio_contact_form' ), __( 'Quiz', 'lc_studio_contact_form' ) );
...
<tbody>
	...
	<tr>
	<th scope="row"><?php echo esc_html( __( 'Questions and answers', 'lc_studio_contact_form' ) ); ?></th>
	<td>
		<fieldset>
		<legend class="screen-reader-text"><?php echo esc_html( __( 'Questions and answers', 'lc_studio_contact_form' ) ); ?></legend>
		<textarea name="values" class="values" id="<?php echo esc_attr( $args['content'] . '-values' ); ?>"></textarea><br />
		<label for="<?php echo esc_attr( $args['content'] . '-values' ); ?>"><span class="description"><?php echo esc_html( __( "One pipe-separated question-answer pair (e.g. The capital of Brazil?|Rio) per line.", 'lc_studio_contact_form' ) ); ?></span></label>
		</fieldset>
	</td>
	</tr>
