...
class WPCF7_ConstantContact extends WPCF7_Service_OAuth2 {
	...
	public function get_title() {
		return __( 'Constant Contact', 'lc_studio_contact_form' );
	}
		...
		switch ( $message ) {
			case 'success':
				echo sprintf(
					...
					esc_html( __( "Connection established.", 'lc_studio_contact_form' ) )
				);
				break;
			case 'failed':
				echo sprintf(
					...
					esc_html( __( "Failed to establish connection. Please double-check your configuration.", 'lc_studio_contact_form' ) )
				);
				break;
			case 'updated':
				echo sprintf(
					'<div class="notice notice-success"><p>%s</p></div>',
					esc_html( __( "Configuration updated.", 'lc_studio_contact_form' ) )
				);
				break;
		}
	}
	...
	public function display( $action = '' ) {
		echo '<p>' . sprintf(
			esc_html( __( 'The Constant Contact integration module allows you to send contact data collected through your contact forms to the Constant Contact API. You can create reliable email subscription services in a few easy steps. For details, see %s.', 'lc_studio_contact_form' ) ),
			wpcf7_link(
				__(
					'https://contactform7.com/constant-contact-integration/',
					'lc_studio_contact_form'
				),
				__( 'Constant Contact integration', 'lc_studio_contact_form' )
			)
		) . '</p>';
		...
		if ( $this->is_active() ) {
			echo sprintf(
				...
				esc_html( __( "This site is connected to the Constant Contact API.", 'lc_studio_contact_form' ) )
			);
		}
...
<tbody>
<tr>
	<th scope="row"><label for="client_id"><?php echo esc_html( __( 'API Key', 'lc_studio_contact_form' ) ); ?></label></th>
	<td><?php
		if ( $this->is_active() ) {
...
<tr>
	<th scope="row"><label for="client_secret"><?php echo esc_html( __( 'App Secret', 'lc_studio_contact_form' ) ); ?></label></th>
	<td><?php
		if ( $this->is_active() ) {
...
<tr>
	<th scope="row"><label for="redirect_uri"><?php echo esc_html( __( 'Redirect URI', 'lc_studio_contact_form' ) ); ?></label></th>
	<td><?php
		echo sprintf(
	...
	?>
	<p class="description"><?php echo esc_html( __( "Set this URL as the redirect URI.", 'lc_studio_contact_form' ) ); ?></p>
	</td>
</tr>
</tbody>
</table>
<?php
		if ( $this->is_active() ) {
			submit_button(
				_x( 'Reset Keys', 'API keys', 'lc_studio_contact_form' ),
				'small', 'reset'
			);
		} else {
			submit_button(
				__( 'Connect to the Constant Contact API', 'lc_studio_contact_form' )
			);
		}
?>
</form>
