...
function wpcf7_submit_form_tag_handler( $tag ) {
	...
	if ( empty( $value ) ) {
		$value = __( 'Send', 'lc_studio_contact_form' );
	}
...
function wpcf7_add_tag_generator_submit() {
	...
	$tag_generator->add( 'submit', __( 'submit', 'lc_studio_contact_form' ),
...
function wpcf7_tag_generator_submit( $contact_form, $args = '' ) {
	...
	$description = __( "Generate a form-tag for a submit button. For more details, see %s.", 'lc_studio_contact_form' );

	$desc_link = wpcf7_link( __( 'https://contactform7.com/submit-button/', 'lc_studio_contact_form' ), __( 'Submit button', 'lc_studio_contact_form' ) );
...
<tbody>
	<tr>
	<th scope="row"><label for="<?php echo esc_attr( $args['content'] . '-values' ); ?>"><?php echo esc_html( __( 'Label', 'lc_studio_contact_form' ) ); ?></label></th>
