...
function wpcf7_sendinblue_editor_panels( $panels ) {
	...
	$editor_panel = function () use ( $prop, $service ) {
		...
		$description = sprintf(
			esc_html(
				__( "You can set up the Sendinblue integration here. For details, see %s.", 'lc_studio_contact_form' )
			),
			wpcf7_link(
				__( 'https://contactform7.com/sendinblue-integration/', 'lc_studio_contact_form' ),
				__( 'Sendinblue integration', 'lc_studio_contact_form' )
			)
		);
...
?>
<h2><?php echo esc_html( __( 'Sendinblue', 'lc_studio_contact_form' ) ); ?></h2>
...
<fieldset>
	...
	<table class="form-table" role="presentation">
		...
		echo esc_html( __( 'Contact lists', 'lc_studio_contact_form' ) );
		...
		echo esc_html(
			__( "Add form submitters to your contact lists", 'lc_studio_contact_form' )
		);
		...
		if ( $lists ) {
			echo sprintf(
				'<legend>%1$s</legend>',
				esc_html( __( 'Select lists to which contacts are added:', 'lc_studio_contact_form' ) )
			);
			...
			echo sprintf(
				'<legend>%1$s</legend>',
				esc_html( __( 'You have no contact list yet.', 'lc_studio_contact_form' ) )
			);
		}
		...
		echo sprintf(
			...
			esc_html( __( 'Manage your contact lists', 'lc_studio_contact_form' ) ),
			esc_html( __( '(opens in a new tab)', 'lc_studio_contact_form' ) )
		);
		...
		echo esc_html( __( 'Welcome email', 'lc_studio_contact_form' ) );
		...
		echo esc_html(
			__( "Send a welcome email to new contacts", 'lc_studio_contact_form' )
		);
		...
		if ( $templates ) {
			echo sprintf(
				'<legend>%1$s</legend>',
				esc_html( __( 'Select an email template:', 'lc_studio_contact_form' ) )
			);
			...
			echo sprintf(
				...
				esc_html( __( '&mdash; Select &mdash;', 'lc_studio_contact_form' ) )
			);
			...
			echo sprintf(
				'<legend>%1$s</legend>',
				esc_html( __( 'You have no active email template yet.', 'lc_studio_contact_form' ) )
			);
		}
		...
		echo sprintf(
			...
			esc_html( __( 'Manage your email templates', 'lc_studio_contact_form' ) ),
			esc_html( __( '(opens in a new tab)', 'lc_studio_contact_form' ) )
		);
...
<?php
	...
	$panels += array(
		'sendinblue-panel' => array(
			'title' => __( 'Sendinblue', 'lc_studio_contact_form' ),
