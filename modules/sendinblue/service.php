...
class WPCF7_Sendinblue extends WPCF7_Service {
	...
	public function get_title() {
		return __( 'Sendinblue', 'lc_studio_contact_form' );
	}
	...
	public function admin_notice( $message = '' ) {
		if ( 'unauthorized' == $message ) {
			echo sprintf(
				...
				esc_html( __( "You have not been authenticated. Make sure the provided API key is correct.", 'lc_studio_contact_form' ) )
			);
		}
	...
	public function display( $action = '' ) {
		echo '<p>' . sprintf(
			esc_html( __( "Store and organize your contacts while protecting user privacy on Sendinblue, the leading CRM & email marketing platform in Europe. Sendinblue offers unlimited contacts and advanced marketing features. For details, see %s.", 'lc_studio_contact_form' ) ),
			wpcf7_link(
		...
		if ( $this->is_active() ) {
			echo sprintf(
				...
				esc_html( __( "Sendinblue is active on this site.", 'lc_studio_contact_form' ) )
			);
		}
...
<tbody>
<tr>
	<th scope="row"><label for="publishable"><?php echo esc_html( __( 'API key', 'lc_studio_contact_form' ) ); ?></label></th>
	<td><?php
		if ( $this->is_active() ) {
...
<?php
		if ( $this->is_active() ) {
			submit_button(
				_x( 'Remove key', 'API keys', 'lc_studio_contact_form' ),
				'small', 'reset'
			);
		} else {
