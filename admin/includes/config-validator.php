...
function wpcf7_admin_warnings_bulk_cv( $page, $action, $object ) {
	...
	$link = wpcf7_link(
		...
		__( 'Validate Contact Form 7 Configuration', 'lc_studio_contact_form' )
	);

	$message = __( "Misconfiguration leads to mail delivery failure or other troubles. Validate your contact forms now.", 'lc_studio_contact_form' );
...
function wpcf7_load_bulk_validate_page( $page, $action ) {
	...
	if ( ! current_user_can( 'wpcf7_edit_contact_forms' ) ) {
		wp_die( __( "You are not allowed to validate configuration.", 'lc_studio_contact_form' ) );
	}
...
<h1><?php echo esc_html( __( 'Validate Configuration', 'lc_studio_contact_form' ) ); ?></h1>
...
<?php
	echo wpcf7_link(
		__( 'https://contactform7.com/configuration-validator-faq/', 'lc_studio_contact_form' ),
		__( 'FAQ about Configuration Validator', 'lc_studio_contact_form' )
	);
?>
