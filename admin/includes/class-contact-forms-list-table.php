...
class WPCF7_Contact_Form_List_Table extends WP_List_Table {
	...
	public static function define_columns() {
		$columns = array(
			...
			'title' => __( 'Title', 'lc_studio_contact_form' ),
			'shortcode' => __( 'Shortcode', 'lc_studio_contact_form' ),
			'author' => __( 'Author', 'lc_studio_contact_form' ),
			'date' => __( 'Date', 'lc_studio_contact_form' ),
		);
	...
	protected function get_bulk_actions() {
		$actions = array(
			'delete' => __( 'Delete', 'lc_studio_contact_form' ),
		);
	...
	public function column_title( $item ) {
		...
		$output = sprintf(
			...
			esc_attr( sprintf(
				...
				__( 'Edit &#8220;%s&#8221;', 'lc_studio_contact_form' ),
				$item->title()
			) ),
			esc_html( $item->title() )
		);
			...
			if ( $count_errors = $config_validator->count_errors() ) {
				$error_notice = sprintf(
					_n(
						...
						$count_errors, 'lc_studio_contact-form' ),
	...
	protected function handle_row_actions( $item, $column_name, $primary ) {
		...
		$actions = array(
			'edit' => wpcf7_link( $edit_link, __( 'Edit', 'lc_studio_contact_form' ) ),
		);
		...
		if ( current_user_can( 'wpcf7_edit_contact_form', $item->id() ) ) {
			...
			$actions = array_merge( $actions, array(
				'copy' => wpcf7_link( $copy_link, __( 'Duplicate', 'lc_studio_contact_form' ) ),
			) );
		}
	...
	public function column_date( $item ) {
		...
		$t_time = sprintf(
			...
			__( '%1$s at %2$s', 'lc_studio_contact_form' ),
			/* translators: date format, see https://www.php.net/date */
			$datetime->format( __( 'Y/m/d', 'lc_studio_contact_form' ) ),
			/* translators: time format, see https://www.php.net/date */
			$datetime->format( __( 'g:i a', 'lc_studio_contact_form' ) )
		);
