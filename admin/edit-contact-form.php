...
function wpcf7_admin_save_button( $post_id ) {
	...
	$button = sprintf(
		...
		esc_attr( __( 'Save', 'lc_studio_contact_form' ) ),
		$onclick );
...
<h1 class="wp-heading-inline"><?php
	if ( $post->initial() ) {
		echo esc_html( __( 'Add New Contact Form', 'lc_studio_contact_form' ) );
	} else {
		echo esc_html( __( 'Edit Contact Form', 'lc_studio_contact_form' ) );
	}
?></h1>
...
<div id="titlediv">
<div id="titlewrap">
	<label class="screen-reader-text" id="title-prompt-text" for="title"><?php echo esc_html( __( 'Enter title here', 'lc_studio_contact_form' ) ); ?></label>
<?php
...
?>
	<p class="description">
	<label for="wpcf7-shortcode"><?php echo esc_html( __( "Copy this shortcode and paste it into your post, page, or text widget content:", 'lc_studio_contact_form' ) ); ?></label>
...
?>
	<p class="description">
	<label for="wpcf7-shortcode-old"><?php echo esc_html( __( "You can also use this old-style shortcode:", 'lc_studio_contact_form' ) ); ?></label>
...
<h3><?php echo esc_html( __( 'Status', 'lc_studio_contact_form' ) ); ?></h3>
<div class="inside">
...
<div class="hidden">
	<input type="submit" class="button-primary" name="wpcf7-save" value="<?php echo esc_attr( __( 'Save', 'lc_studio_contact_form' ) ); ?>" />
</div>
...
?>
	<input type="submit" name="wpcf7-copy" class="copy button" value="<?php echo esc_attr( __( 'Duplicate', 'lc_studio_contact_form' ) ); ?>" <?php echo "onclick=\"this.form._wpnonce.value = '$copy_nonce'; this.form.action.value = 'copy'; return true;\""; ?> />
<?php endif; ?>
...
<div id="delete-action">
	<input type="submit" name="wpcf7-delete" class="delete submitdelete" value="<?php echo esc_attr( __( 'Delete', 'lc_studio_contact_form' ) ); ?>" <?php echo "onclick=\"if (confirm('" . esc_js( __( "You are about to delete this contact form.\n  'Cancel' to stop, 'OK' to delete.", 'lc_studio_contact_form' ) ) . "')) {this.form._wpnonce.value = '$delete_nonce'; this.form.action.value = 'delete'; return true;} return false;\""; ?> />
</div><!-- #delete-action -->
<?php endif; ?>
...
<h3><?php echo esc_html( __( "Do you need help?", 'lc_studio_contact_form' ) ); ?></h3>
<div class="inside">
	<p><?php echo esc_html( __( "Here are some available options to help solve your problems.", 'lc_studio_contact_form' ) ); ?></p>
	<ol>
		<li><?php echo sprintf(
			/* translators: 1: FAQ, 2: Docs ("FAQ & Docs") */
			__( '%1$s and %2$s', 'lc_studio_contact_form' ),
			wpcf7_link(
				__( 'https://contactform7.com/faq/', 'lc_studio_contact_form' ),
				__( 'FAQ', 'lc_studio_contact_form' )
			),
			wpcf7_link(
				__( 'https://contactform7.com/docs/', 'lc_studio_contact_form' ),
				__( 'docs', 'lc_studio_contact_form' )
			)
		); ?></li>
		<li><?php echo wpcf7_link(
			__( 'https://wordpress.org/support/plugin/lc_studio_contact_form/', 'lc_studio_contact_form' ),
			__( 'Support forums', 'lc_studio_contact_form' )
		); ?></li>
		<li><?php echo wpcf7_link(
			__( 'https://contactform7.com/custom-development/', 'lc_studio_contact_form' ),
			__( 'Professional services', 'lc_studio_contact_form' )
		); ?></li>
	</ol>
</div>
</div><!-- #informationdiv -->
...
<div class="keyboard-interaction"><?php
	echo sprintf(
		...
		esc_html( __( '%1$s %2$s keys switch panels', 'lc_studio_contact_form' ) ),
		'<span class="dashicons dashicons-leftright" aria-hidden="true"></span>',
		sprintf(
			'<span class="screen-reader-text">%s</span>',
			/* translators: screen reader text */
			esc_html( __( '(left and right arrow)', 'lc_studio_contact_form' ) )
		)
	);
?></div>
...
<?php
	...
	if ( current_user_can( 'wpcf7_edit_contact_form', $post_id ) ) {
		$panels = array(
			'form-panel' => array(
				'title' => __( 'Form', 'lc_studio_contact_form' ),
				'callback' => 'wpcf7_editor_panel_form',
			),
			'mail-panel' => array(
				'title' => __( 'Mail', 'lc_studio_contact_form' ),
				'callback' => 'wpcf7_editor_panel_mail',
			),
			'messages-panel' => array(
				'title' => __( 'Messages', 'lc_studio_contact_form' ),
		...
		$panels['additional-settings-panel'] = array(
			'title' => $additional_settings
				? sprintf(
					...
					__( 'Additional Settings (%d)', 'lc_studio_contact_form' ),
					$additional_settings )
				: __( 'Additional Settings', 'lc_studio_contact_form' ),
