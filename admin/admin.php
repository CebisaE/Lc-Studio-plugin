...
function wpcf7_admin_menu() {
	...
	add_menu_page(
		__( 'Contact Form 7', 'lc_studio_contact_form' ),
		__( 'Contact', 'lc_studio_contact_form' )
	...
	$edit = add_submenu_page( 'wpcf7',
		__( 'Edit Contact Form', 'lc_studio_contact_form' ),
		__( 'Contact Forms', 'lc_studio_contact_form' )
	...
	$addnew = add_submenu_page( 'wpcf7',
		__( 'Add New Contact Form', 'lc_studio_contact_form' ),
		__( 'Add New', 'lc_studio_contact_form' )
	...
	if ( $integration->service_exists() ) {
		$integration = add_submenu_page( 'wpcf7',
			__( 'Integration with External API', 'lc_studio_contact_form' ),
			__( 'Integration', 'lc_studio_contact_form' )
...
function wpcf7_admin_enqueue_scripts( $hook_suffix ) {
	...
	wp_enqueue_style( 'lc_studio_contact_form-admin',
		wpcf7_plugin_url( 'admin/css/styles.css' ),
		array(), WPCF7_VERSION, 'all'
	);

	if ( wpcf7_is_rtl() ) {
		wp_enqueue_style( 'lc_studio_contact_form-admin-rtl',
	...
	$args = array(
		'apiSettings' => array(
			'root' => esc_url_raw( rest_url( 'lc_studio_contact_form/v1' ) ),
			'namespace' => 'lc_studio_contact_form/v1',
		...
		'saveAlert' => __(
			...
			'lc_studio_contact_form' ),
		'activeTab' => isset( $_GET['active-tab'] )
			? (int) $_GET['active-tab'] : 0,
		'configValidator' => array(
			'errors' => array(),
			'howToCorrect' => __( "How to resolve?", 'lc_studio_contact_form' ),
			'oneError' => __( '1 configuration error detected', 'lc_studio_contact_form' ),
			'manyErrors' => __( '%d configuration errors detected', 'lc_studio_contact_form' ),
			'oneErrorInTab' => __( '1 configuration error detected in this tab panel', 'lc_studio_contact_form' ),
			'manyErrorsInTab' => __( '%d configuration errors detected in this tab panel', 'lc_studio_contact_form' ),
			'docUrl' => WPCF7_ConfigValidator::get_doc_link(),
			/* translators: screen reader text */
			'iconAlt' => __( '(configuration error)', 'lc_studio_contact_form' ),
		),
	);
...
function wpcf7_load_contact_form_admin() {
	...
	if ( 'save' == $action ) {
		...
		if ( ! current_user_can( 'wpcf7_edit_contact_form', $id ) ) {
			wp_die(
				__( "You are not allowed to edit this item.", 'lc_studio_contact_form' )
			);
		}
	...
	if ( 'delete' == $action ) {
		...
		foreach ( $posts as $post ) {
			...
			if ( ! current_user_can( 'wpcf7_delete_contact_form', $post->id() ) ) {
				wp_die(
					__( "You are not allowed to delete this item.", 'lc_studio_contact_form' )
				);
			}

			if ( ! $post->delete() ) {
				wp_die( __( "Error in deleting.", 'lc_studio_contact_form' ) );
			}
...
<h1 class="wp-heading-inline"><?php
	echo esc_html( __( 'Contact Forms', 'lc_studio_contact_form' ) );
?></h1>
...
<?php
	...
	if ( ! empty( $_REQUEST['s'] ) ) {
		echo sprintf(
			...
			. __( 'Search results for &#8220;%s&#8221;', 'lc_studio_contact_form' )
			. '</span>',
			esc_html( $_REQUEST['s'] )
		);
	}
...
<form method="get" action="">
	...
	<?php $list_table->search_box( __( 'Search Contact Forms', 'lc_studio_contact_form' ), 'wpcf7-contact' ); ?>
...
<h1><?php echo esc_html( __( 'Integration with External API', 'lc_studio_contact_form' ) ); ?></h1>

<p><?php
	echo sprintf(
		/* translators: %s: link labeled 'Integration with external APIs' */
		esc_html( __( "You can expand the possibilities of your contact forms by integrating them with external services. For details, see %s.", 'lc_studio_contact_form' ) ),
		wpcf7_link(
			__( 'https://contactform7.com/integration-with-external-apis/', 'lc_studio_contact_form' ),
			__( 'Integration with external APIs', 'lc_studio_contact_form' )
		)
	);
?></p>
...
function wpcf7_admin_updated_message( $page, $action, $object ) {
	...
	if ( 'created' == $_REQUEST['message'] ) {
		$updated_message = __( "Contact form created.", 'lc_studio_contact_form' );
	} elseif ( 'saved' == $_REQUEST['message'] ) {
		$updated_message = __( "Contact form saved.", 'lc_studio_contact_form' );
	} elseif ( 'deleted' == $_REQUEST['message'] ) {
		$updated_message = __( "Contact form deleted.", 'lc_studio_contact_form' );
	}
	...
	if ( 'failed' == $_REQUEST['message'] ) {
		$updated_message =
			__( "There was an error saving the contact form.", 'lc_studio_contact_form' );
		...
		if ( $count_invalid ) {
			$updated_message = sprintf(
				_n(
					...
					$count_invalid, 'lc_studio_contact_form'
				),
			...
			$updated_message = __( "Configuration validation completed. No invalid contact form was found.", 'lc_studio_contact_form' );
...
function wpcf7_plugin_action_links( $links, $file ) {
	...
	$settings_link = wpcf7_link(
		...
		__( 'Settings', 'lc_studio_contact_form' )
	);
...
<p><?php
	echo sprintf(
		...
		__( '<strong>Contact Form 7 %1$s requires WordPress %2$s or higher.</strong> Please <a href="%3$s">update WordPress</a> first.', 'lc_studio_contact_form' ),
		WPCF7_VERSION,
...
function wpcf7_not_allowed_to_edit( $page, $action, $object ) {
	...
	$message = __( "You are not allowed to edit this contact form.", 'lc_studio_contact_form' );
